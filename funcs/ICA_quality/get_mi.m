...
%
% Revised for parfor implementation by Seyed Yahya Shirazi 06/2023 UCSD, INC, SCCN
...
if dox == 0
    MI_cell = cell(nu, 1);  % Initialize MI as cell array
    parfor i = 1:nu
        MI_temp = zeros(nu, nlags*2 + 1);  % Temporary variable for storing MI in the current iteration
        for j = (i+1):nu
            if nlags == 0
                pmf2r = diff([0 find(diff(sort(u(i,:)+nbins*(u(j,:)-1)))) Nu])/Nu;
                H2 = -sum(pmf2r.*log(pmf2r));
                MI_temp(j) = Hu(i) + Hu(j) - H2;
            else
                % do zero lag
                pmf2r = diff([0 find(diff(sort(u(i,1:Nu)+nbins*(u(j,1:Nu)-1)))) Nu])/Nu;
                H2 = -sum(pmf2r.*log(pmf2r));
                MI_temp(j,nlags+1) = Hu(i,nlags+1) + Hu(j,nlags+1) - H2;

                % do +g and -g lags
                for g = 1:nlags
                    pmf2r = diff([0 find(diff(sort(u(i,1+g:Nu)+nbins*(u(j,1:(Nu-g))-1)))) Nu-g])/(Nu-g);
                    H2 = -sum(pmf2r.*log(pmf2r));
                    MI_temp(j,nlags+1+g) = Hu(i,nlags+1+g) + Hu(j,nlags+1-g) - H2;
        ...
        MI_cell{i} = MI_temp;  % Store the temporary MI result in the cell array
    end
    ...
    MI_cell = cell(nu, 1);  % Initialize MI as a cell array
    parfor i = 1:nu
        MI_temp = zeros(nx, nlags*2 + 1);  % Temporary variable for storing MI in the current iteration
        for j = 1:nx
            if nlags == 0
                pmf2r = diff([0 find(diff(sort(u(i,:)+nbins*(x(j,:)-1)))) Nu])/Nu;
                H2 = -sum(pmf2r.*log(pmf2r));
                MI_temp(j) = Hu(i) + Hx(j) - H2;
            else
                for g = 0:nlags
                    pmf2r = diff([0 find(diff(sort(u(i,1+g:Nu)+nbins*(x(j,1:(Nu-g))-1)))) Nu-g])/(Nu-g);
                    H2 = -sum(pmf2r.*log(pmf2r));
                    MI_temp(j,nlags+1+g) = Hu(i,nlags+1+g) + Hx(j,nlags+1-g) - H2;
