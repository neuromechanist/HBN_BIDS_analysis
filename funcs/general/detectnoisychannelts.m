...
for n=1:szdata(1)
    ...
    i=find(dist<30 & dist>0); % was .5 before here the distance between electrodes is defined to look for correlated data; .1 = 10 cm
%     i=find(dist<.25 & dist>0); % was .5 before here the distance between electrodes is defined to look for correlated data; .1 = 10 cm
    IND{n}=i;
end

numt=floor(szdata(2)/EEG.nbchan);
...
for t=1:numt
    %c_vec=[];
  %  t
  data=EEG.data(:,[(EEG.nbchan*t -(EEG.nbchan-1)):EEG.nbchan*t]);
