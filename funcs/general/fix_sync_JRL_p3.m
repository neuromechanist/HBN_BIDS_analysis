...
% p3 = rising edge code, delete first triggers for CSV and EEG

function [new_eeg_trig_latencies new_csv_trig_latencies EEG new_biomech] = fix_sync_JRL_p3(EEG, csv_filename,csv_data, csv_sync_ch_idx, csv_sample_rate)
...
min_num_events = min([length(eeg_trig_latencies) length(csv_trig_latencies) ...
    length(new_csv_trig_latencies) length(new_eeg_trig_latencies)]);

eeg_trig_latencies = eeg_trig_latencies(1:min_num_events);
csv_trig_latencies = csv_trig_latencies(1:min_num_events);
new_eeg_trig_latencies = new_eeg_trig_latencies(1:min_num_events);
new_csv_trig_latencies = new_csv_trig_latencies(1:min_num_events);
